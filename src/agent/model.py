...
from src.agent.utils.config import ServerEnv, initialize_environment
...
# Try to import ADK models for ThinkingLiteLlm
try:
...
def create_model(model_name: str, model_provider: str) -> Any:
    ...
    if model_provider == "LOCAL":
        if ThinkingLiteLlm is None:
            logger.error(
                "LiteLlm (google-adk) not available, cannot use LOCAL provider."
            )
            return model_name
